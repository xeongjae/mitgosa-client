...
리뷰 분석 결과 UI 구성을 위해 다음 6가지 (`pros`, `cons`, `ratio`, `summary`, `size`, `recommendation`) 데이터가 필요했습니다.
각 필드별 명확한 형식 지정 (배열, 문자열, 글자수 제한) 친근한 말투 요청으로 타겟 사용자 경험을 고려하였습니다.

```javascript
const prompt = `
다음은 패션 제품에 대한 고객 리뷰입니다. 이 리뷰들을 분석하여 객체 형식으로 요약해주세요.
- 말투는 온라인 쇼핑몰 리뷰처럼 딱딱하지 않고 솔직한 느낌으로 써주세요.
...
```javascript
const prompt = `
다음은 패션 제품에 대한 고객 리뷰입니다. 이 리뷰들을 분석하여 아래 JSON 형식으로 요약해주세요.
- 말투는 온라인 쇼핑몰 리뷰처럼 딱딱하지 않고 솔직한 느낌으로 써주세요.
- 다른 텍스트 없이 JSON 형식만 응답해주세요.
