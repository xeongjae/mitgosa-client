...
### 2. 가상 스크롤 크롤링 문제 : Puppeteer의 기능을 활용한 해결

초기에 Puppeteer를 사용하여 리뷰를 수집한 결과, 10-20개 정도 소수의 리뷰만 수집되는 문제가 발생했습니다. <br />
개발자 도구를 분석한 결과, 예상했던 무한스크롤 방식이 아닌 **가상 스크롤(Virtual Scroll) 방식**으로 리뷰를 렌더링하고 있었습니다.

> **가상 스크롤: 사용자가 스크롤할 때마다 새로운 데이터를 동적으로 로드하는 방식.**

무신사의 경우 순차적으로 한 번에 20-25개 정도의 리뷰만 렌더링됩니다. 이는 한 번의 크롤링으로 모든 리뷰를 수집할 수 없음을 의미했습니다.
<br /> 따라서 Puppeteer의 자동 스크롤 기능을 활용하여 **단계적 스크롤 → 로딩 대기 → 데이터 수집**을 반복하는 로직을 구현했습니다.

테스트 결과, 실제 리뷰 총 개수 대비 약 95-98%의 리뷰를 성공적으로 수집할 수 있었습니다. 소량의 누락은 네트워크 지연이나 동적 로딩 타이밍 이슈로 인한 것으로 분석되었으나, 분석 속도와 정확성 간의 균형을 고려하여 현재 수준에서 만족할 만한 결과라고 판단했습니다.
 
다음은 위 내용을 토대로 진행한 실험의 결과입니다.

<details>
<summary>[ 크롤링 테스트 ]</summary>

![리뷰 데이터 수집 정확도 테스트](public/review-test.png)
